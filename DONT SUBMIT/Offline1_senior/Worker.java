...
public class Worker extends Thread {
    ...
    {
        this.socket = socket;
        Client_IP = socket.getRemoteSocketAddress();
    }
...
//                }
    ...
    public String receiveFile(String type, ObjectInputStream in, ObjectOutputStream out) {
        try {
            ...
            System.out.println(student_id + " wants to upload a " + type + " file " + filename + " of size : " + filesize + " bytes");
            ...
            currentFile = "FileServer/" + student_id + "/" + type + "/" + filename;
...
//                }
    ...
    public void run()
    {
        ...
        try {
            ...
            while (active)
            {
                student_id = (String) in.readObject();
                boolean success = Server.login(student_id, Client_IP);
                out.writeObject(success);
                if(success == false){
                    continue;
                }
                ...
                while(true){
                    ...
                    if( option.equalsIgnoreCase("1") ){
                        // LookUp Users
                        System.out.println(student_id + " requested for user list");
                        // get active users
                        ...
                        for(int i = 0 ; i < types.length ; i++){
                            ...
                            results.add(types[i] + " :");
                            File[] files = new File("FileServer/" + student_id + "/" + types[i] + "/" ).listFiles();
                            //If this pathname does not denote a directory, then listFiles() returns null.
                            for (File file : files) {
                                if (file.isFile()) {
                                    int fileid = Server.get_file_id("FileServer/" + student_id + "/" + types[i] + "/" + file.getName());
                        ...
                        if(download.equalsIgnoreCase("1")){
                            ...
                            String filepath = Server.get_file_path(fileid);
                            System.out.println(student_id + " wants to download " + filepath);
                            if( filepath == null ){
                                System.out.println("No such file in your directory");
                                out.writeObject("There is no such file in this name in your folders");
                                continue;
                            }
                            if(filepath.startsWith("FileServer/" + student_id + "/public/") | filepath.startsWith( "FileServer/" + student_id + "/private/")){
                        ...
                        if( exists == true ){
                            // user with id exists
                            System.out.println(student_id + " wants to see files of " + id);
                            List<String> results = new ArrayList<String>();
                            // send all public files of student id to client
                            File[] files = new File("FileServer/" + id + "/public/" ).listFiles();
                            //If this pathname does not denote a directory, then listFiles() returns null.
                            for (File file : files) {
                                if (file.isFile()) {
                        ...
                        // requesting a file
                        System.out.println(student_id + " wants to request for a file.");
                        String description = (String) in.readObject();
                        Server.addRequest(student_id, description);
                        ...
                        for(int i = 0 ; i < req.size() ; i++ ){
                            ...
                            if(req.get(i).getRequester().equalsIgnoreCase(student_id)){
                        ...
                        Server.addUpload(req_id, student_id, "FileServer/" + student_id + "/public/" + filename);
                    }
                        ...
                        out.close();
                        socket.close();
                        Server.logout(student_id);
                        System.out.println(student_id + " has logged out");
                        active = false;
                        break;
                    }
                }
            ...
            if(currentFile != null){
                ...
                boolean success = to_delete.delete();
                if(success == true){
                    System.out.println(currentFile + " deleting due to " + student_id + " going offline.");
                }
                else{
            ...
            System.out.println(student_id + " has gone offline, Logging out");
            Server.logout(student_id);
        }
    }
}
