...
private:
    ...
    std::string last_found_scope_id ;
...
int destructor_calls = 0;
    ...
    void EnterScope()
    {
        ...
        if (current_scope == nullptr) {
            ...
            new_id = current_scope->id + "." + std::to_string(current_scope->child_count);
        }
    ...
    bool Insert(std::string symbol_name, std::string type)
    {
    ...
    void print_current_scope_table(std::ostream& out)
    {
        current_scope->print(out, 1);
    }


    void print_all_scope_table2(std::ostream& out) {
        ScopeTable *temp = current_scope;
        int i = 1;
        while(temp != NULL){
            temp->print(out,i);
