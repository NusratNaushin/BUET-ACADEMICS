...
from sklearn.datasets import load_iris     #python er librarry ... classical dataset guli dewa acheyyey.. function hishabe dewa ase
...
def load_data():
    ...
    iris = load_iris()     #purano bikkhato dataset
    df = pd.DataFrame(data=iris.data, columns=iris.feature_names)
    df['target'] = iris.target    #classification gula
...
def preprocess_data(df):     #dataset model e dewar age upojukto korbo
    """Handle nulls, duplicates, and prepare features"""
    print("\n" + "="*50)
    print("Step 2: Preprocessing data...")
    
    # Check for null values
    null_count = df.isnull().sum().sum()  #kotogulo null ache?
    print(f"Null values found: {null_count}")
    if null_count > 0:
        df = df.dropna()    #null drop krow
    ...
    if duplicate_count > 0:
        df = df.drop_duplicates()    #duplicate drop krow
...
def main():
    ...
    # Step 1: Load data
    df, target_names = load_data()     #DATASET LOAD DIYE START
